...
import { add_message } from '../store/messageSlice';

export const ChatInput = ({ messages, setMessages, onSendMessage, sessionId }) => {
  ...
  const handleSendMessage = async () => {
    ...
    try {
      if (onSendMessage) {
        await onSendMessage(currentInput, setMessages, setIsLoading);
      } else {
        ...
        if (apiUrl) {
          const res = await fetch(`${apiUrl}/chat`, {
            ...
            body: JSON.stringify({ message: currentInput, user_id, session_id: sessionId }),
          });
        ...
        dispatch(add_message({ message: { content: data.reply }}));
      ...
      setMessages((prev) => {
        ...
        newMessages[newMessages.length - 1] = {
          role: 'assistant',
          content: "Sorry, you reached your today's limit.",
        };
        return newMessages;
      });
    } finally {
  ...
  return (
    <div className="flex flex-col h-screen bg-slate-950 font-sans">
      ...
      {/* Input */}
      <div className="bg-slate-950 border-slate-900/50 p-4 md:p-6 mb-15">
